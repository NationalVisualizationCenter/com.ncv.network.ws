...
namespace NativeWebSocket
{
  ...
  public class WebSocket : IWebSocket {
    ...
    public Task Send (ArraySegment<byte> data) {
      int ret = WebSocketSend (this.instanceId, data.ToArray(), data.Count);
    ...
    public class WebSocket : IWebSocket
    {
        ...
        public Task Send(ArraySegment<byte> bytes)
        {
            // return m_Socket.SendAsync(buffer, WebSocketMessageType.Binary, true, CancellationToken.None);
            return SendMessage(sendBytesQueue, WebSocketMessageType.Binary, bytes);
        }
